...
export const DemoShowroomScreen: FC<DemoTabScreenProps<"DemoShowroom">> = function DemoShowroomScreen(
  _props,
) {
      ...
      >
        ...
        <SectionCard testID="cta-card">
          ...
          <Button
            ...
            style={themed($ctaButtonOrange)}
            accessibilityRole="button"
            accessibilityLabel={translate("demoShowroomScreen:planTripCta")}
            testID="cta-compute-trip"
          >
            <View style={[themed($ctaBtnInner), isRTL ? { flexDirection: "row-reverse" } : null]}>
              <Text
                preset="bold"
                style={themed($ctaBtnText)}
                tx="demoShowroomScreen:planTripCta" // i18n key should map to "Compute planned trip cost"
              />
              <Icon
                icon="back"
                size={16}
                // style={themed($ctaBtnIcon)}
                // color controlled by text color for consistency
              />
            </View>
          </Button>
        </SectionCard>
 ...
 * Styles
 */
const $ctaButtonOrange: ThemedStyle<ViewStyle> = () => ({
  marginTop: 8, // aligns with spacing.sm but avoids extra theme import inside constants
  backgroundColor: "#FF8A00", // orange like the reference
  borderRadius: 999,
  paddingVertical: 12,
  paddingHorizontal: 14,
})

const $ctaBtnInner: ThemedStyle<ViewStyle> = () => ({
  flexDirection: "row",
  alignItems: "center",
...
const $ctaBtnText: ThemedStyle<TextStyle> = () => ({
  // brown-ish, readable over orange; tweak to fit your theme if desired
  color: "#3B2A00",
})
