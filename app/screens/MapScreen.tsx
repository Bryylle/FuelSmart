  ...
  const renderFuelToggle = (label: string, boolKey: keyof ReportData, nameKey: keyof ReportData, placeholder: string) => (
    <View style={styles.mt_12}>
      <View style={styles.toggleRow}>
        <Text text={label} style={styles.flex} />
        <TouchableOpacity 
          disabled={isExistingBrand} 
          onPress={() => setReportData({ ...reportData, [boolKey]: !reportData[boolKey] })}
          style={[styles.toggleBtn, reportData[boolKey] && styles.toggleBtnActive, isExistingBrand && styles.opacity_half]}
        >
          <Text text={reportData[boolKey] ? "YES" : "NO"} size="xs" style={styles.inline_004} />
        </TouchableOpacity>
      </View>
  ...
  return (
    <Screen contentContainerStyle={styles.flex}>
          ...
          {isFilterVisible && (
            <Animated.View entering={FadeInUp} exiting={FadeOutUp} style={styles.filterDropdown}>
              ...
              <>
                ...
                <TouchableOpacity style={styles.brandPickerTrigger} onPress={() => { 
              ...
              <>
                <View style={styles.inline_019}>
                  <TouchableOpacity style={[styles.pendingFormBtns, styles.inline_020]} onPress={handleCancelFilters}><Text style={styles.inline_021}>Cancel</Text></TouchableOpacity>
                  <TouchableOpacity style={[styles.pendingFormBtns, styles.inline_022]} onPress={handleApplyAll}><Text style={styles.inline_004}>Apply Filters</Text></TouchableOpacity>
                </View>
              </>
            </Animated.View>
          )}
        ...
        <View style={styles.brandModalOverlay}>
          <View style={styles.brandModalContent}>
            <View style={styles.brandModalHeader}>
              ...
              <TouchableOpacity style={styles.inline_023} onPress={() => setModalBrands(modalBrands.length === availableBrands.length ? [] : [...availableBrands])}>
                <Text size="xs" style={styles.inline_024}>Select All</Text>
                <View style={[styles.checkbox, modalBrands.length === availableBrands.length && styles.checkboxActive]}>
                  {modalBrands.length === availableBrands.length && <Icon icon="check" size={10} color="white" />}
                </View>
              </TouchableOpacity>
            </View>
            <ScrollView style={styles.inline_025}>
              {availableBrands.map(brand => (
                <TouchableOpacity key={brand} style={styles.brandOption} onPress={() => setModalBrands(prev => prev.includes(brand) ? prev.filter(b => b !== brand) : [...prev, brand])}>
                  <Text style={modalBrands.includes(brand) ? styles.brandTextSelected : styles.brandTextDefault}>
                    {brand}
                  </Text>
                  <View style={[styles.checkbox, modalBrands.includes(brand) && styles.checkboxActive]}>
                    {modalBrands.includes(brand) && <Icon icon="check" size={10} color="white" />}
                  </View>
                </TouchableOpacity>
              ))}
            </ScrollView>
            <View style={styles.inline_026}>
              <TouchableOpacity style={[styles.pendingFormBtns, styles.inline_020]} onPress={() => setIsBrandPickerVisible(false)}><Text style={styles.inline_021}>Cancel</Text></TouchableOpacity>
              <TouchableOpacity style={[styles.pendingFormBtns, styles.inline_022]} onPress={() => { 
                  setTempBrands([...modalBrands])
                  setIsBrandPickerVisible(false)
                }}>
                <Text style={styles.inline_027}>Apply</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
                  ...
                  <View style={styles.inline_029}>
                    <View style={styles.flex}>
                      ...
                      <View style={styles.inline_030}>
                        <Text size="sm" style={styles.opacity_half}>
                ...
                {!selectedStation.isPending && (
                  <View style={styles.stationDetailBtnWrapper}>
                    <View style={styles.stationDetailBtnRow}>
                      {isReporting ? (
                        <>
                          <TouchableOpacity style={[styles.stationDetailModalBtns, styles.inline_051]} onPress={() => setIsReporting(false)}><Text style={styles.buttonText}>Cancel</Text></TouchableOpacity>
                          <TouchableOpacity style={[styles.stationDetailModalBtns, styles.inline_022]} onPress={handleUpdatePrice}><Text style={styles.buttonText}>Submit</Text></TouchableOpacity>
                        </>
                      ) : (
                        <>
                          <TouchableOpacity style={[styles.stationDetailModalBtns, selectedStation.fetchError && styles.inline_052]} disabled={selectedStation.fetchError} onPress={showDirections}><Icon icon="directions" color="white" size={24} /><Text style={styles.buttonText}>Directions</Text></TouchableOpacity>
                          <TouchableOpacity style={[styles.stationDetailModalBtns, styles.inline_022, selectedStation.fetchError && styles.inline_052]} disabled={selectedStation.fetchError} onPress={() => setIsReporting(true)}><Icon icon="price_update" color="white" size={24} /><Text style={styles.buttonText}>Update Price</Text></TouchableOpacity>
                        </>
                      )}
                    </View>
                  </View>
      ...
      {region.latitudeDelta < ZOOM_THRESHOLD && !isAddMarkerMode && (
        <TouchableOpacity 
          style={[styles.utilityBtn, { bottom: 120, backgroundColor: isAddMarkerMode ? colors.palette.angry500 : "#1737ba" }]} 
      ...
      {isAddMarkerMode && (
        <Animated.View entering={FadeInUp} style={styles.addMarkerToolTip}>
          ...
          <View style={styles.inline_062}>
            <TouchableOpacity style={styles.confirmBtn} onPress={() => toggleAddMarkerMode()}>
              <Text text="Cancel" style={styles.inline_027} />
            </TouchableOpacity>
            <TouchableOpacity style={styles.confirmBtn} onPress={() => setReportModalVisible(true)}>
        ...
        <View style={styles.brandModalOverlay}>
          <View style={[styles.brandModalContent, styles.inline_063]}>
            ...
            <ScrollView style={styles.inline_064} showsVerticalScrollIndicator={false}>
              ...
              <TouchableOpacity 
                style={styles.brandPickerTrigger} 
              ...
              >
                <View style={styles.brandModalOverlay}>
                  <View style={styles.brandModalContent}>
                    <View style={styles.searchBrandModalHeader}>
                    ...
                    <ScrollView style={styles.inline_065}>
                      {filteredBrandOptions.map((brand) => {
                        ...
                        return (
                          ...
                          >
                            <Text style={isNew ? styles.brandTextSelected : styles.brandTextDefault}>
                              {brand} {isNew ? "(Add New)" : ""}
                            </Text>
              ...
              <Text preset="formLabel" text="Municipality/City" style={styles.inline_066} />
              <TouchableOpacity 
                activeOpacity={0.7}
                style={styles.selectTrigger} 
                onPress={() => setIsMuniPickerVisible(true)}
              >
                <View style={styles.inline_023}>
                  <Icon icon="search" size={20} color={colors.palette.neutral500} style={styles.inline_024} />
                  <Text 
                    style={reportData.city ? styles.muniTriggerTextSelected : styles.muniTriggerTextPlaceholder} 
                  ...
                  <View style={styles.brandModalContent}>
                    ...
                    >
                      {muniSearchQuery.length < 2 ? (
                        ...
                        <View style={styles.inline_068}>
                          <Text style={styles.inline_070}>No results found.</Text>
                        </View>
                      )}
                    </ScrollView>
                  </View>
              ...
              <Text text="Available Fuel Types" weight="bold" style={styles.inline_071} />
            ...
            <View style={styles.inline_026}>
              <TouchableOpacity style={styles.pendingFormBtns} onPress={() => setReportModalVisible(false)}>
                <Text text="Cancel" />
              </TouchableOpacity>
              <TouchableOpacity 
                style={[styles.pendingFormBtns, styles.inline_022]} 
      ...
      {isNotAtMarkerLevel && !isAddMarkerMode && (
        <TouchableOpacity 
          style={[styles.utilityBtn, styles.inline_073]} 
      ...
      {region.latitudeDelta < ZOOM_THRESHOLD && !isAddMarkerMode && /* TODO: Need to add AND isMapMounted  */ (
        <TouchableOpacity style={[styles.utilityBtn, styles.inline_074]} onPress={toggle3DMapView}>
...
const styles = StyleSheet.create({
  ...
  searchBar: { backgroundColor: "white", height: 50, borderRadius: 25, flexDirection: "row", alignItems: "center", paddingHorizontal: 15, elevation: 5 },
  searchPlaceholder: { flex: 1, marginLeft: 10, color: "#8E8E93", fontSize: 13 },
  filterDropdown: { backgroundColor: "white", marginTop: 10, borderRadius: 20, padding: 20, elevation: 5 },
  segmentedControl: { flexDirection: "row", backgroundColor: "#F2F2F7", borderRadius: 10, padding: 2, marginTop: 8 },
  segment: { flex: 1, paddingVertical: 8, alignItems: "center", borderRadius: 8 },
  segmentActive: { backgroundColor: "white", elevation: 2 },
  fuelTypeSegment: { fontSize: 12, color: colors.palette.neutral400},
  fuelTypeSegmentActive: { color: colors.palette.primary500, fontWeight: "bold" },
  brandPickerTrigger: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#F2F2F7', borderRadius: 10, padding: 12, marginTop: 8 },
  brandModalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center' },
  brandModalContent: { backgroundColor: 'white', width: '85%', borderRadius: 20, padding: 20 },
  brandModalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15 },
  brandOption: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 14, borderBottomWidth: HAIRLINE, borderBottomColor: '#EEE' },
  checkbox: { width: 18, height: 18, borderRadius: 4, borderWidth: 1, borderColor: '#D1D1D6', justifyContent: 'center', alignItems: 'center' },
  checkboxActive: { backgroundColor: colors.palette.primary500, borderColor: colors.palette.primary500 },
  pendingFormBtns: { flex: 1, paddingVertical: 12, borderRadius: 25, alignItems: 'center' },
  ...
  detailCard: { backgroundColor: "white", height: 350 },
  dismissHandle: { borderTopLeftRadius: 20, borderTopRightRadius: 20, alignItems: 'center', paddingVertical: 5, backgroundColor: '#605e5e' },
  innerContent: { paddingHorizontal: 20, paddingTop: 12 },
  favoriteBtn: { padding: 8 },
  priceDashboard: { backgroundColor: "#F2F2F7", borderRadius: 16, marginTop: 10, height: 160, borderWidth: 1, borderColor: "#E5E5EA" },
  ...
  priceInput: { color: colors.palette.primary500, fontSize: 13, fontWeight: "700", borderBottomWidth: 1, borderBottomColor: colors.palette.primary500, textAlign: 'center', minWidth: 50 },
  stationDetailBtnWrapper: { position: "absolute", bottom: 0, left: 0, right: 0, padding: 20, backgroundColor: "white" },
  stationDetailBtnRow: { flexDirection: "row", justifyContent: "space-between" },
  stationDetailModalBtns: { flexDirection: "row", backgroundColor: "#605e5e", paddingVertical: 10, borderRadius: 20, width: '48%', justifyContent: "center", alignItems: 'center' },
  buttonText: { color: "white", marginLeft: 8, fontWeight: "600", fontSize: 14 },
  priceBadgeContainer: {
    alignItems: 'center',
    justifyContent: 'center',
    padding: 5,
  ...
  inline_012: { marginStart: 5 },
  inline_014: { flexDirection: 'row', justifyContent: 'space-between' },
  inline_015: { color: colors.palette.primary500 },
  ...
  inline_020: { backgroundColor: '#F2F2F7' },
  inline_021: { color: 'black' },
  inline_022: { backgroundColor: colors.palette.primary500 },
  inline_023: { flexDirection: 'row', alignItems: 'center' },
  inline_024: { marginRight: 8 },
  inline_025: { maxHeight: 350 },
  ...
  inline_029: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  ...
  inline_061: { color: 'white', marginBottom: 7 },
  inline_062: {flex: 1, flexDirection: "row", padding: 10, justifyContent: "space-between", width: "90%", alignItems: "center"},
  inline_063: { width: '90%' },
  inline_064: { maxHeight: 450, marginTop: 10 },
  inline_065: { maxHeight: 400 },
  inline_066: { marginBottom: 4 },
  inline_067: { maxHeight: 450, marginTop: 8 },
  ...
  inline_071: { marginTop: 20, marginBottom: 5 },
  inline_072: { color: 'white' },
  inline_073: {bottom : 50},
  inline_074: { bottom: 190 },
